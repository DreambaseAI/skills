---
name: dreambase-mcp
description: Operate the Dreambase MCP server correctly — orientation and call order (whoami → list_workspaces → list_dashboards/list_datasets → query_dataset), the DuckDB dialect and result caps on query_dataset, connection discovery before building any API/MCP source, polling create_health_report with get_health_report, retry rules for non-idempotent writes, and what FORBIDDEN vs NOT_FOUND mean. Use whenever any Dreambase tool is available or called — whoami, list_workspaces, list_dashboards, list_datasets, get_dataset, query_dataset, list_aggregates, get_aggregate, list_connections, get_connection, create_health_report, list_health_reports, get_health_report, list_skills, get_skill, create_skill, update_skill — or when the user asks about their Dreambase workspace, dashboards, datasets, metrics, database health, or connected Supabase/API/MCP data sources.
---
